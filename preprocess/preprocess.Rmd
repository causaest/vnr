...
Confirmation of pace as provided in the dataset.
...
conf_pace <- function(results) {
  ...
  z$pace_secs_orig <- as.integer(sapply(speed_split, `[[`, 2))
...
write_vnr_processed <- function(year) {
  results <- year |> read_vnr_raw() |> fix_times() |> calc_time() |> final_fix()
  path_to_processed <- paste0("../raw/vnr-results-processed-", year, ".rds")
